...
class Solution {
    int[][] dd = new int[][] {{0,1},{0,-1},{-1,0},{-1,1},{-1,-1},{1,1},{1,0},{1,-1}};
    ...
    private void dfs(char[][] board, int x,int y) {
        ...
        if (board[x][y] == 'E') {
            for (int i = 0;i < 8;i++) {
                ...
                board[x][y] = mines > 0 ? (char) (mines + '0') : 'B';
                if (board[x][y] == 'B') {
                    dfs(board,x + dd[i][0],dd[i][1] + y);
                }
            }
        }
    }
    ...
    private int getMines(char[][] board,int x,int y) {
        int res = 0;
        for (int i = 0;i < 8;i++) {
            int dx = dd[i][0] + x,dy = dd[i][1] + y;
            if (!isOutOfBounds(board,dx,dy) && board[dx][dy] == 'M')
                res++;
        }
        return res;
    }
